...
class FirebaseSemesterSummaryRepositoryImpl(
    ...
    override suspend fun incPresent(by: Long): Result<Unit> =
        runCatching {
            summaryDocument()
                .update("presentRecords", FieldValue.increment(by))
                .await()
        }
    ...
    override suspend fun incAbsent(by: Long): Result<Unit> =
        runCatching {
            summaryDocument().update("absentRecords", FieldValue.increment(by)).await()
        }

    override suspend fun decAbsent(by: Long): Result<Unit> =
        runCatching {
            Log.d("tag", "x decrementing absent")
            val doc = summaryDocument()
                .update("absentRecords", FieldValue.increment(-by))

            Log.d("tag", "x obtained doc ref")
            doc.await()
            Log.d("tag", "x decremented absent")
            Unit
        }.onFailure { Log.d("tag", "error decrementing sem absent: $it") }

    override suspend fun incUnmarked(by: Long): Result<Unit> =
        runCatching {
            summaryDocument()
                .update("unmarkedRecords", FieldValue.increment(by))
                .await()
    ...
    override suspend fun incCancelled(by: Long): Result<Unit> =
        runCatching {
            summaryDocument()
                .update("cancelledRecords", FieldValue.increment(by))
                .await()
        }
